 ...
 */
class moodle1_mod_sadkofile_handler extends moodle1_mod_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'sadkofile', '/MOODLE_BACKUP/COURSE/MODULES/MOD/RESOURCE',
                array(
                    'renamefields' => array(
                        'summary' => 'intro',
                    ),
    ...
    public function process_resource(array $data, array $raw) {
        ...
        // no successor is interested in this record, convert it to the new mod_sadkofile (aka File module)
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_sadkofile');
        ...
        // write resource.xml
        $this->open_xml_writer("activities/sadkofile_{$moduleid}/resource.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'sadkofile', 'contextid' => $contextid));
        $this->write_xml('sadkofile', $resource, array('/sadkofile/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/sadkofile_{$currentcminfo['id']}/inforef.xml");
     ...
     */
    public function on_sadkofile_end(array $data) {
        if ($successor = $this->get_successor($data['type'], $data['reference'])) {
            $successor->on_legacy_sadkofile_end($data);
        }
    }
    ...
    protected function get_successor($type, $reference) {
        ...
        switch ($type) {
            ...
            case 'file':
                ...
                break;
            default:
                throw new moodle1_convert_exception('unknown_sadkofile_successor', $type);
        }
