...
/**
 * Defines backup_sadkofile_activity_task class
 *
...
require_once($CFG->dirroot . '/mod/sadkofile/backup/moodle2/backup_sadkofile_stepslib.php');

/**
 * Provides the steps to perform one complete backup of the Resource instance
 */
class backup_sadkofile_activity_task extends backup_activity_task {

    /**
     * @param bool $resourceoldexists True if there are records in the sadkofile_old table.
     */
    ...
    protected function define_my_steps() {
        $this->add_step(new backup_sadkofile_activity_structure_step('sadkofile_structure', 'sadkofile.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        if (static::$resourceoldexists === null) {
            static::$resourceoldexists = $DB->record_exists('sadkofile_old', array());
        }

        // If there are links to items in the sadkofile_old table, rewrite them to be links to the correct URL
        ...
        if (static::$resourceoldexists) {
            ...
            if ($result) {
                ...
                $oldrecs = $DB->get_records_select('sadkofile_old', "cmid $insql", $params, '', 'cmid, newmodule');

                for ($i = 0; $i < count($matches[0]); $i++) {
                    $cmid = $matches[2][$i];
                    if (isset($oldrecs[$cmid])) {
                        // sadkofile_old item, rewrite it
            ...
            // Resource ID links.
            list($insql, $params) = $DB->get_in_or_equal($matches[2]);
            $oldrecs = $DB->get_records_select('sadkofile_old', "oldid $insql", $params, '', 'oldid, cmid, newmodule');
