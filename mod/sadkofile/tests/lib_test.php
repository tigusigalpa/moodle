...
/**
 * Unit tests for mod_sadkofile lib
 *
 ...
 * @since      Moodle 3.0
 */
class mod_sadkofile_lib_testcase extends advanced_testcase {
     ...
     * @return void
     */
    public function test_sadkofile_view() {
        global $CFG;
        ...
        $resource = $this->getDataGenerator()->create_module('sadkofile', array('course' => $course->id),
                                                            array('completion' => 2, 'completionview' => 1));
        $context = context_module::instance($resource->cmid);
        $cm = get_coursemodule_from_instance('sadkofile', $resource->id);

        // Trigger and capture the event.
        $sink = $this->redirectEvents();

        sadkofile_view($resource, $course, $cm, $context);
        ...
        $this->assertInstanceOf('\mod_sadkofile\event\course_module_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $moodleurl = new \moodle_url('/mod/sadkofile/view.php', array('id' => $cm->id));
    ...
    /**
     * Tests the sadkofile_get_coursemodule_info function.
     *
    ...
    public function test_get_coursemodule_info() {
        ...
        $resource1 = $generator->create_module('sadkofile', array('course' => $course->id,
        ...
        $resource2 = $generator->create_module('sadkofile', array('course' => $course->id,
        ...
        $fs->create_file_from_string($filerecord, 'Test');
        $resource3 = $generator->create_module('sadkofile', array('course' => $course->id,
                'name' => 'R3', 'files' => $draftid));

        // Try get_coursemodule_info for first one.
        $info = sadkofile_get_coursemodule_info(
    ...
    public function test_sadkofile_core_calendar_provide_event_action() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $actionevent = mod_sadkofile_core_calendar_provide_event_action($event, $factory);
    ...
    public function test_sadkofile_core_calendar_provide_event_action_already_completed() {
        global $CFG;
    ...
    /**
     * Test mod_sadkofile_core_calendar_provide_event_action with user override
     */
    public function test_sadkofile_core_calendar_provide_event_action_user_override() {
        global $CFG, $USER;
        ...
        // Decorate action event.
        $actionevent = mod_sadkofile_core_calendar_provide_event_action($event, $factory, $USER->id);

        // Decorate action with a userid override.
        $actionevent2 = mod_sadkofile_core_calendar_provide_event_action($event, $factory, $user->id);
