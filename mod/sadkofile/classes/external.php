 ...
 * @since      Moodle 3.0
 */
class mod_sadkofile_external extends external_api {
     ...
     * @since Moodle 3.0
     */
    public static function view_sadkofile_parameters() {
        return new external_function_parameters(
            array(
                'resourceid' => new external_value(PARAM_INT, 'sadkofile instance id')
            )
        );
    }

    /**
     * Simulate the sadkofile/view.php web interface page: trigger events, completion, etc...
     *
    ...
    public static function view_resource($resourceid) {
        ...
        $params = self::validate_parameters(self::view_sadkofile_parameters(),
                                            array(
        ...
        $resource = $DB->get_record('sadkofile', array('id' => $params['resourceid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($resource, 'sadkofile');
     ...
     * @since Moodle 3.0
     */
    public static function view_sadkofile_returns() {
        ...
        if (!empty($params['courseids'])) {
            ...
            foreach ($resources as $resource) {
                ...
                list($resource->intro, $resource->introformat) = external_format_text($resource->intro,
                                                                $resource->introformat, $context->id, 'mod_sadkofile', 'intro', null);
                $resource->introfiles = external_util::get_area_files($context->id, 'mod_sadkofile', 'intro', false, false);
                $resource->contentfiles = external_util::get_area_files($context->id, 'mod_sadkofile', 'content');
